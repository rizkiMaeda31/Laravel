...
<html lang="en">
    <head>
        ...
        <title>Delete Artist {{ $current->Name }}</title>
    </head>
    ...
    <body class="container mt-5">
        <form method="POST" action="{{url('api/chinook/artist')}}" id="form">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="title" name="name" readonly value="{{$current->Name}}">
        ...
        document.getElementById('form').addEventListener('submit', function (event) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var newTab = window.open('{{ url('view/chinook/artist') }}/', '_self');
                    if (newTab) {
                        newTab.focus();
                    } else {
                        // Handling for browsers with pop-up blockers
                        window.location.href = '{{ url('view/chinook/artist') }}/';
                    }
                },
                error: function () {

...
<body>    
            ...
            <div class="container-fluid">
                <div class="collapse navbar-collapse" id="mynavbar">
                    <ul class="navbar-nav me-auto">
                        <li class="nav-item">
                            <span class="nav-link" >Total rows: <span style="color: crimson" id="count">{{ count($tracks) }} </span> rows</span>
                        </li>
                        <li class="nav-item">
                            <a type="button" class=" nav-link" href="{{ url('view/track/create') }}">Create New Track</a>
                        </li>
                    ...
                    <form class="d-flex" action="{{ url('view/track/find') }}">
                        <input class="form-control me-2" type="text" placeholder="Search" name="s" id="s">
                        <div class="btn-group">
                            <button class="btn btn-primary" type="submit" ><i class="fa fa-search"></i></button>
                            <a class="btn btn-primary" href="{{ url('view/track/') }}">Reset</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="container mt-4">
            <div class="row justify-content-center">
                <div class="col-md-12" style="width: 100%; overflow-x: auto;">
                    <table class="table table-bordered table-hover " style="width: auto" id="trackTable">
                        <thead class="table-success ">
                            <tr>
                                @foreach ($columns as $index => $column)
                                    <th class="text-center {{ $index === 0 ? 'text-nowrap' : '' }} sortable" data-column="{{ $index }}">{{ $column }}</th>
                                @endforeach
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($tracks as $track)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a type="button" class="btn btn-warning" href="{{ url('view/track/update/'.$track->TrackId) }}">Update</a>
                                            <a type="button" class="btn btn-danger" href="{{ url('view/track/delete/'.$track->TrackId) }}">Delete</a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            var ascending = true;
            var currentColumn = 0;
            $('#trackTable th.sortable').click(function () {
                // alert($(this).index());
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#trackTable tbody').html(data.res);
                        $('#count').html(data.count);
